...
export function DashboardPage({ filteredTrackers, stats, connectionChart, riskRanking, topDrivers, executiveCards, offlineTrackers, anomalyTrackers, filter, setFilter }) {
  return <>
    ...
    <section className="dashboard-grid premium-grid phase2-grid">
      ...
      <div className="panel"><div className="panel-header"><div><h3>Anomalies terrain</h3><p>Unités avec signaux à surveiller</p></div><Link className="ghost-btn" to="/analytics">Voir analytics</Link></div><div className="alerts-list">{anomalyTrackers.slice(0,5).map((tracker) => <Link key={tracker.id} className="alert-row link-row" to={`/tracker/${tracker.id}`}><div className="alert-icon"><Activity size={16} /></div><div><strong>{tracker.label}</strong><p>{tracker.employeeName}</p><span>{tracker.events.length} événements · risque {tracker.riskScore}</span></div></Link>)}</div></div>
    </section>
  </>
}
