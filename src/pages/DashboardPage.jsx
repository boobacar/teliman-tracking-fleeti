...
export function DashboardPage({ filteredTrackers, stats, connectionChart, riskRanking, topDrivers, executiveCards, offlineTrackers, anomalyTrackers, filter, setFilter }) {
  return <>
    ...
    <section className="dashboard-grid premium-grid phase2-grid">
      <div className="panel panel-large"><div className="panel-header"><div><h3>Camions à surveiller en priorité</h3><p>Classement par score de vigilance</p></div></div><div className="alerts-list">{riskRanking.slice(0, 5).map((tracker) => <div key={tracker.id} className="alert-row"><div className="alert-icon"><ShieldAlert size={16} /></div><div><strong>{tracker.label}</strong><p>{tracker.employeeName}</p><span>Score de vigilance {tracker.riskScore} · Excès de vitesse {tracker.eventCounts.speedup || 0}</span></div></div>)}</div></div>
      <div className="panel"><div className="panel-header"><div><h3>Chauffeurs les plus actifs</h3><p>Classement par kilométrage du jour</p></div></div><div className="driver-ranking">{topDrivers.map((driver, index) => <div key={`${driver.name}-${index}`} className="driver-rank-row"><strong>#{index + 1}</strong><div><span>{driver.name}</span><small>{driver.tracker}</small></div><div><span>{driver.mileage} km</span><small>Score de vigilance {driver.risk}</small></div></div>)}</div></div>
    </section>
    <section className="dashboard-grid premium-grid phase2-grid">
      ...
      <div className="panel"><div className="panel-header"><div><h3>Anomalies terrain</h3><p>Unités avec signaux à surveiller</p></div><Link className="ghost-btn" to="/analytics">Voir analytics</Link></div><div className="alerts-list">{anomalyTrackers.slice(0,5).map((tracker) => <Link key={tracker.id} className="alert-row link-row" to={`/tracker/${tracker.id}`}><div className="alert-icon"><Activity size={16} /></div><div><strong>{tracker.label}</strong><p>{tracker.employeeName}</p><span>{tracker.events.length} événements · score de vigilance {tracker.riskScore}</span></div></Link>)}</div></div>
    </section>
  </>
}
